...
class RegisterController: LBTAFormController {
    ...
    let fullNameTextField = IndentedTextField(placeholder: "Nombre completo", padding: 24, cornerRadius: 25)
    let emailTextField = IndentedTextField(placeholder: "Email", padding: 24, cornerRadius: 25)
    let passwordTextField = IndentedTextField(placeholder: "Contraseña", padding: 24, cornerRadius: 25, isSecureTextEntry: true)
    lazy var signUpButton = UIButton(title: "Registrarse", titleColor: .white, font: .boldSystemFont(ofSize: 18), backgroundColor: .black, target: self, action: #selector(handleSignup))
    
    let errorLabel = UILabel(text: "Ocurrio un error durante el registro.", font: .systemFont(ofSize: 14), textColor: .red, textAlignment: .center, numberOfLines: 0)
    
    lazy var goBackButton = UIButton(title: "Regresar a login.", titleColor: .black, font: .systemFont(ofSize: 16), target: self, action: #selector(goToRegister))
    ...
    @objc fileprivate func handleSignup() {
        ...
        hud.textLabel.text = "Registrando"
        hud.show(in: view)
